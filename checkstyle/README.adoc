...
You can override used Checkstyle version by explicitly configuring the dependency directly under the plugin. This is sometimes necessary as `maven-checkstyle-plugin` lags behind in version it depends on internally. Do note that the newer versions can depend on higher version of Java and cause it to not be a drop-in upgrade.
