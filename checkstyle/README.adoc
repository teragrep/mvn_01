...
==== How to override used Checkstyle version

You can override used Checkstyle version by explicitly configuring the dependency directly under the plugin. This is sometimes necessary as `maven-checkstyle-plugin` lags behind in the version it depends on internally.
...
See https://mvnrepository.com/artifact/com.puppycrawl.tools/checkstyle/versions[maven central] for newest versions and https://checkstyle.sourceforge.io/#JRE_and_JDK[checkstyle homepage] for their support status.
